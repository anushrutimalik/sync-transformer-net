...
class SizeInvariantTimeSformer(nn.Module):
    ...
    def forward(self, x, mask = None,  identities_mask = None, size_embedding = None, positions = None):
        ...
        if self.enable_pos_emb:
            x += self.pos_emb(positions.long())
        else:
            x += (self.pos_emb(torch.arange(x.shape[1], dtype=torch.long, device=device)))
        ...
        if mask is not None:
            ...
            if identities_mask is not None:
        ...
        if self.require_attention:
            return self.to_out(cls_token), attentions, x
        else:
            return self.to_out(cls_token), x
