...
class CrossModalAttention(nn.Module):
    def __init__(self, visual_dim: int, audio_dim: int, num_heads: int = 8, dropout: float = 0.1):
        """
        ...
        self.audio_proj = nn.Linear(audio_dim, visual_dim) if audio_dim != visual_dim else nn.Identity()
...
class SyncWeldNet(nn.Module):
    def __init__(self, timesformer_config, audio_model_name='facebook/wav2vec2-large-xlsr-53', num_classes=1):
        """
        ...
        self.efficient_net_block = timesformer_config['model'].get('efficient-net-block', 7)
        ...
        self.audio_dim = self.audio_engine.config.hidden_size # usually 1024 for Large-XLSR
        
        # --- Cross-Modal Welding Layer ---
        self.cross_modal_fusion = CrossModalAttention(visual_dim=self.visual_dim, audio_dim=self.audio_dim)
    ...
    def forward(self, visual_x, audio_waveforms, mask=None, identities_mask=None, size_embedding=None, positions=None):
        """
        ...
        features = self.efficient_net.extract_features_at_block(visual_frames, self.efficient_net_block) # [B*F, Channels, P_H, P_W]
        ...
        fused_latents, _ = self.cross_modal_fusion(visual_tokens=visual_features, audio_latents=audio_latents)
